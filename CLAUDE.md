...
- **Web hydration:** the static clock is stale by definition. The root hydrates with fixed inputs (light theme, fallback language) and remounts live in a layout effect, never `useEffect`, which runs after paint. App routes pre-render nothing (the tabs layout gates on `useIsHydrated()`). City pages pre-render with the build time, read back from their `data-t` stamp so hydration matches exactly, and keep it invisible until live. Keep i18n init synchronous, since an async gate empties every static page. Anything touching storage or `useSyncExternalStore` must survive the Node static render: storage has a server stand-in, and stores need a server snapshot.
...
- **Web service worker:** production web builds register `/sw.js` (`scripts/build-pwa.ts`). If a web change "doesn't show up" in a production preview, the old worker is serving the old cache until the new one activates: reload once, or clear it in DevTools → Application. Dev (`expo start`) never registers one. A new file the offline app needs must be under `/_expo` or `/assets`, or be added to the precache list.
