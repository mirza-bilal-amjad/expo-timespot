...
- **Head.** Title, description, canonical, Open Graph (with the image, its size and alt text), `twitter:card` `summary_large_image`, JSON-LD (`WebPage` + `Place`, `dateModified` = build time, `primaryImageOfPage`).
- **OG image (task 6.6).** `scripts/build-og-images.ts` renders `/og/<slug>.png`, 1200 × 630, after the export: satori lays it out, resvg rasterises it. It shows the city, the country, the year's offsets ("UTC−5 · UTC−4 with DST", from `getYearOffsets`), the zone, and a clock face. ~~a representative clock face~~ **clarified:** the face is fixed at 10:10, and the eyebrow reads "Local time in", not "Current time". A share image outlives its build by months, so it must never show a reading. Long names step down in size to stay within two lines. Colours come from the theme tokens. English only, like the page.
- **Not yet.** `hreflang` is n/a: the static pages are English only and switch language client-side. Store links join the CTA with 7.4; today the CTA opens the web app.
