...
| 6.9 | Deploy to EAS Hosting with a custom domain | LCP < 1.2 s p75 on a throttled 4G profile. **Prepared 2026-09-26, not deployed:** the container can't reach `expo.dev` and has no Expo login. `npm run deploy:web` and `npm run check:deploy` are ready (`12` §6). LCP, measured on the production export with brotli and 4× CPU throttling, median of 3: **repeat visit 0.5 s** on both profiles (the service worker serves the shell); **cold 1.8 s on fast 4G, 5.2 s on slow 4G**. ~~< 1.2 s~~ **corrected:** met on repeat visits, not on a cold one. The LCP element is the clock or date, which by design stay invisible until the app runs (a stale time is never shown), so a cold LCP is the time to download and run ~620 KB of compressed JS, the framework floor from 6.1. What would move it: a smaller city-page bundle (Expo Router has one entry today), or a server that renders the live time (ADR-0006 rules out a server). |
