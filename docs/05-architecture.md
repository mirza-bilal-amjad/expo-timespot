...
- **Hydration correctness (task 6.5, done).** Two kinds of route:
  - **App routes** (`/`, `/clock`, `/map`) are per-visitor: saved cities in local storage, the current time, the browser's language. None of it is pre-rendered. The root layout gates on `useIsHydrated()`, which is `false` during the static render and the hydration render and flips in a `useLayoutEffect` (before paint). The server HTML and the hydration pass therefore both render nothing, and the real UI mounts before the first paint. This gives no mismatch by construction and never shows a build-time clock. `+html.tsx` paints the page background per `prefers-color-scheme`, and an inline script applies a stored theme override before first paint, so the empty page is already the right colour. i18n init is synchronous (`initImmediate: false`), which removes the old async gate that made every static page an empty shell. Verified on the production export: real hydration, zero console warnings, both themes.
  - **SEO routes** (`/time/[slug]`, task 6.3) do pre-render real content. There, the clock node renders `suppressHydrationWarning` and the real value is set in a `useLayoutEffect`.
...
|---|---|---|
| Cold start → readable clock | < 900 ms p75 | fonts preloaded; map code-split off the initial route; boot dataset packed to 136 KB gz, search names on demand |
| Web initial JS (gz) | ~~< 180 KB~~ **/ 756 · /clock 747 · /map 815 KiB**, guarded by `npm run size:web` | **Corrected 2026-09-26 (task 6.1).** 180 KB was never reachable. The framework floor alone (Expo Router + react-native-web + React DOM entry ≈ 277 KB gz, plus Reanimated and Gesture Handler) exceeds it before any app code, and the boot dataset is another 136 KB. What was cut, from 1,128 KB on `/`: async routes (map + topology 74 KB only on `/map`); the search names (115 KB), search sheet (26 KB) and other sheets loaded on first use; the dataset repacked (327 → 136 KB gz); the dev-only Stories route out of production; web stand-ins for Keyboard Controller, `expo-device` and the plural-rules polyfill; Ignite's unused date-fns loader removed. Expo's `EXPO_UNSTABLE_TREE_SHAKING` was tried: −34 KB (Reanimated's CSS engine is side-effectful), not worth an unstable flag. |
