...
| Output (app) | `cities.core.json` (boot data, columnar, 136 KB gz) + `cities.search.json` (search names, loaded on demand), packed from `cities.min.json` by `scripts/pack-cities.ts` (task 6.1) |
